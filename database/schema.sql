...
-- Additive evidence catalog for technical News. Existing News remains unchanged.
...
create table if not exists news_technical_evidence (
  id text primary key, product_slug text not null, claim_label text not null, value_text text not null,
  test_conditions text, standard_reference text, source_document text not null, source_locator text not null,
  report_number text, evidence_level text not null check (evidence_level in ('public_exact', 'public_scoped', 'context_only')),
  public_status boolean not null default true, restriction_note text, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists news_technical_topics (
  id text primary key, title text not null, excerpt text not null, primary_product_slug text not null,
  evidence_ids text[] not null default '{}', tags text[] not null default '{}',
  status text not null default 'pending' check (status in ('pending', 'publishing', 'published', 'blocked')),
  sort_order integer not null default 100, published_article_id uuid references news_articles(id) on delete set null,
