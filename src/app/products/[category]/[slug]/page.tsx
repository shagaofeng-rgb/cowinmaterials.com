...
export default async function ProductDetailPage({ params }: ProductPageProps) {
  ...
  return <><Header /><main>
    ...
    <ContentPanels label={`${product.name} content sections`}>
      <ContentPanel id="overview" label="Overview"><div className="panel-section product-detail-grid"><article><h2>Overview</h2>{profile.overview.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</article><article><h2>Applications</h2><div className="chip-row">{product.applications.map((item) => <span className="chip" key={item}>{item}</span>)}</div></article><article><h2>Last reviewed</h2><p>{profile.lastReviewed}</p><p className="proof-note">Information is presented for technical evaluation, not as a project guarantee.</p></article></div></ContentPanel>
      <ContentPanel id="specifications" label="Specifications"><div className="panel-section muted"><div className="data-scope-heading"><span className="eyebrow">Technical evidence</span><h2>Key specifications</h2><p>{profile.testScope}</p></div><div className="technical-fact-grid">{profile.facts.map((fact) => <article key={fact.label}><span>{fact.label}</span><strong>{fact.value}</strong><p>{fact.scope}</p></article>)}</div></div></ContentPanel>
      <ContentPanel id="handling" label="Handling & limits"><div className="panel-section product-detail-grid"><article><h2>Handling / storage</h2><ul className="metric-list">{profile.handling.map((item) => <li key={item}><CheckCircle2 size={16} /><span>{item}</span></li>)}</ul></article><article><h2>Selection limits</h2><ul className="metric-list">{profile.selectionLimits.map((item) => <li key={item}><CheckCircle2 size={16} /><span>{item}</span></li>)}</ul></article><article><h2>Related applications</h2><div className="chip-row">{relatedApplications.map((item) => <Link className="chip" href={`/applications/${item.slug}`} key={item.slug}>{item.shortTitle}</Link>)}</div></article></div></ContentPanel>
      <ContentPanel id="documents" label="Documents"><div className="panel-section document-panel"><h2>Source documents</h2><ul className="metric-list">{profile.sourceDocuments.map((item) => <li key={item}><FileText size={16} /><span>{item}</span></li>)}</ul><Link className="text-link" href={`/request-quote?request=Request%20Technical%20Data&product=${encodeURIComponent(product.name)}`}>Request applicable documentation<ArrowRight size={16} /></Link></div></ContentPanel>
      <ContentPanel id="faq" label="FAQ"><div className="panel-section"><h2>Product FAQ</h2><div className="faq-list">{profile.faq.map(([question, answer]) => <article key={question}><h3>{question}</h3><p>{answer}</p></article>)}</div></div></ContentPanel>
    </ContentPanels>
