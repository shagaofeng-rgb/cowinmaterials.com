...
export default function LocationsPage() {
  return <><Header /><main><section className="page-hero compact"><SectionHeading level={1} eyebrow="Locations" title="Office location and contact" intro="Use the Quzhou office details for project correspondence, document requests and commercial communication." /></section><section className="section location-grid"><article className="location-card"><MapPin size={24} /><span className="eyebrow">Office</span><h2>Quzhou office</h2><p><a href={site.officeMapUrl} target="_blank" rel="noopener noreferrer">{site.officeAddress}</a></p><p>Telephone: <a href={site.whatsappUrl} target="_blank" rel="noopener noreferrer" data-analytics-whatsapp-placement="location_phone">{site.phone}</a><br />Email: <a href={`mailto:${site.email}`}>{site.email}</a></p></article><OfficeMap /></section></main><Footer /></>;
}
