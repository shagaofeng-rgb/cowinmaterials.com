...
export function hasAerogelMaterialResearchContext(candidate: Pick<NewsCandidate, "title" | "summary" | "keywords">) {
  const text = candidateText(candidate);
  return /\baerogels?\b/i.test(text)
    && /\b(silane|silica|porous|nanoporous|sorbent|sorption|capture|coating|thermal|insulation|blanket|particle|granule|slurry)\b/i.test(text);
}
...
export function hasDirectMaterialRelevance(candidate: Pick<NewsCandidate, "title" | "summary" | "keywords">) {
  ...
  const relevantAerogelTopic = hasAerogelMaterialResearchContext(candidate) || (/\baerogel\b/i.test(text) && /\b(silica|insulation|thermal|fire|battery|building|coating|blanket)\b/i.test(text));
