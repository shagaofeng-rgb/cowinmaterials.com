...
type NewsRow = {
  id: string; title: string; slug: string; excerpt: string; content_html: string; status: NewsArticle["status"]; category: string | null; tags: string[] | null;
...
export async function getPublishedNewsSitemapSummary() {
  ...
  try { const result = await pool.query<{ count: string; last_modified: Date | string | null }>(`select count(*)::text as count, max(updated_at) as last_modified from news_articles where status = 'published' and deleted_at is null and published_at <= now() and cover_image_url <> ''`); return { count: Number(result.rows[0]?.count || 0), lastModified: iso(result.rows[0]?.last_modified) }; } catch { return { count: 0, lastModified: new Date().toISOString() }; }
}
